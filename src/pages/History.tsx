...
const History = () => {
  ...
  useEffect(() => {
    ...
    getHistory(user.uid).then((h) => { setItems(h); setLoading(false); }).catch(() => setLoading(false));
  }, [user]);

  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        <h1 className="text-2xl font-display font-bold flex items-center gap-2"><Clock size={24} /> Watch History</h1>
      </motion.div>

      {!user && <p className="text-muted-foreground">Sign in to see your watch history.</p>}

      {loading ? (
        <div className="flex justify-center py-20"><div className="w-8 h-8 border-2 border-primary border-t-transparent rounded-full animate-spin" /></div>
      ) : items.length === 0 ? (
        <p className="text-center text-muted-foreground py-20">No watch history yet</p>
      ) : (
